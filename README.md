...
- Keeps UART command polling enabled when VBUS is detected, the battery monitor
  reports charging, or the battery is full at 100%.

## Release 0.8.24 Highlights

- Keeps UART command polling active when the battery monitor reports charging,
  even if GPIO24 VBUS reads low.
...
- Continued source organization by extracting UART command parsing and prompt
  handling into `src/app/uart_commands.*`.

## Release 0.8.19 Highlights

- Continued source organization by extracting the clock help display into
  `src/clock/clock_help.*`.

## Release 0.8.18 Highlights

- Continued source organization by extracting screenshot capture tone handling
  into `src/app/screenshot_service.*`.

## Release 0.8.17 Highlights

- Continued source organization by extracting Life runtime, drawing, initial
  pattern selection, and hourly-run tracking into `src/life/life_runtime.*`.

## Release 0.8.16 Highlights

- Continued source organization by extracting the settings editor model,
  drawing, and key helpers into `src/app/settings_editor.*`.

## Release 0.8.15 Highlights

- Continued source organization by extracting the set-time editor model,
  drawing, and key helpers into `src/app/set_time_editor.*`.

## Release 0.8.14 Highlights

- Continued source organization by extracting common and digital clock
  rendering helpers into `src/clock/clock_render.*`.

## Release 0.8.13 Highlights

- Continued source organization by extracting alarm editor and ringing screen
  drawing into `src/alarm/alarm_ui.*`.

## Release 0.8.12 Highlights

- Continued source organization by extracting alarm matching and label helpers
  into `src/alarm/alarm_model.*`.

## Release 0.8.11 Highlights

- Continued source organization by extracting EEPROM-backed settings model and
  store helpers into `src/settings/`.

## Release 0.8.10 Highlights

- Continued source organization by extracting startup probe, battery read, and
  backlight state helpers into `src/platform/`.

## Release 0.8.9 Highlights

- Started the `main.cpp` source organization work by extracting pure clock
  date/time helpers into `src/clock/clock_time.*`.
